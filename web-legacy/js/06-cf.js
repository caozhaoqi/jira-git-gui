...
async function queryCfLogs() {
  ...
  try {
    ...
    CF_LAST_LOG_RESULT = {
      ...
      total,
      rows,
      localPage: 1,   // 全量排序后的本地页码（每次新查询重置）
      raw: res,
    };
    ...
    // 默认按时间降序展示：先拉取剩余分页合并为全量，避免「只排当前页」
    // （平台分页返回顺序不一定按时间，仅排当前页会把最新日志推到后面的页）
    try {
      await ensureAllCfLogs();
    } catch (_) { /* 拉取失败时降级：对已加载的数据排序并提示 */ }
...
async function ensureAllCfLogs() {
  ...
  const token = document.getElementById('cf-token').value.trim();
  const proxy = document.getElementById('cf-proxy').value.trim();
  // 拉全量时用较大的 page_size 减少请求次数（平台接受大值，本地分页仍按用户设置）
...
function renderCfResults() {
  ...
  // 分页：已加载全量时走「本地分页」（第一页为最新），否则展示平台当前页
  const pageSize = base.page_size || 200;
  let localPage = base.localPage || 1;
  let display = filtered;
  let isLocalPaged = false;
  let totalPages = 1;
  ...
  // 分页：全量本地分页（第一页最新）或服务端分页（仅未全量时）
  const pageIndex = base.page_index || 1;
  const total = base.total != null ? base.total : all.length;
  if (isLocalPaged) {
    html += '<div class="cf-pagination">';
    if (localPage > 1) {
      html += `<button class="btn btn-sm" onclick="CF_LAST_LOG_RESULT.localPage=${localPage - 1};renderCfResults()">上一页</button>`;
    }
    html += `<span style="font-size:12px;color:var(--muted)">${localPage} / ${totalPages}（本地，第 1 页为最新）</span>`;
    if (localPage < totalPages) {
      html += `<button class="btn btn-sm" onclick="CF_LAST_LOG_RESULT.localPage=${localPage + 1};renderCfResults()">下一页</button>`;
    }
    html += '</div>';
  } else if (!isFull) {
...
// 云函数日志搜索栏：实时过滤 + 时间排序
document.getElementById('cf-search-input').addEventListener('input', () => {
  // 搜索条件变化时回到第一页（本地分页下）
  if (CF_LAST_LOG_RESULT) CF_LAST_LOG_RESULT.localPage = 1;
  renderCfResults();
});
