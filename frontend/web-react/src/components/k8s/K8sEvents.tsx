  ...
  return (
    <div className="k8s-events">
      <div className="k8s-ev-cfg">
        <input className="input input-sm" value={ns} onChange={(e) => setNs(e.target.value)} placeholder={t('k8s.events.nsPh')} />
        <input className="input input-sm" value={kind} onChange={(e) => setKind(e.target.value)} placeholder={t('k8s.events.kindPh')} />
        <input className="input input-sm" value={name} onChange={(e) => setName(e.target.value)} placeholder={t('k8s.events.namePh')} />
        <input className="input input-sm" type="number" value={limit} onChange={(e) => setLimit(Number(e.target.value))} style={{ width: 90 }} />
        <label className="chk"><input type="checkbox" checked={allNs} onChange={(e) => setAllNs(e.target.checked)} /> {t('k8s.events.allNs')}</label>
        <label className="chk"><input type="checkbox" checked={auto} onChange={(e) => setAuto(e.target.checked)} /> {t('k8s.events.auto')}</label>
        <button className="btn btn-sm" onClick={load} disabled={busy || !target.env}>{t('k8s.events.refresh')}</button>
      </div>
